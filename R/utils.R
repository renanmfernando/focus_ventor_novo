http_getter <- function(url, verbose = getOption("rbcb_verbose", default = FALSE)) {
  ...
  res <- tryCatch({
    GET(url = url, config = list(), handle = h, if (verbose) verbose())
  }, error = function(e) NULL)
  
  # Se a primeira tentativa falhar e o URL for HTTPS, tenta HTTP
  if (is.null(res) || status_code(res) != 200) {
    if (grepl("^https://", url, ignore.case = TRUE)) {
      url_http <- sub("^https://", "http://", url, ignore.case = TRUE)
      res <- tryCatch({
        GET(url = url_http, config = list(), handle = h, if (verbose) verbose())
      }, error = function(e) NULL)
    }
  }
  ...
  if (is.null(res) || status_code(res) != 200) {
    msg <- sprintf("Request error %s\n", if (!is.null(res)) status_code(res) else "Unknown")
    stop(msg, "URL:", url)
  }
